<template>
  <v-menu :offset-y="true" transition="scroll-y-transition" bottom left>
    ...
    <v-list>
      <v-list-item :to="{ name: 'admin.deposits.show', params: { id } }" exact>
        <v-list-item-icon>
      ...
      <v-list-item v-if="['coinpayments', 'ethereum'].indexOf(code) > -1" :to="{ name: 'admin.deposits.transaction', params: { id } }" exact>
        <v-list-item-icon>
